...
describe('BuysService', () => {
    ...
    describe('listAllBuys', () => {
        it('should return all buys with pagination info', async () => {
            ...
            const mockData = [{ id: 1, item: 'Item 1' }, { id: 2, item: 'Item 2' }];
            const totalCount = 2;
        ...
        it('should handle repository errors', async () => {
            ...
            buysRepository.listAll = jest.fn().mockRejectedValue(new Error('Database error'));
            buysRepository.countAll = jest.fn().mockResolvedValue(0);

            await expect(buysService.listAllBuys(filters)).rejects.toThrow('Database error');
        });
    });
});