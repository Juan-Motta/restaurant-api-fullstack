...
import { getRecipeController, listAllRecipesController } from '../../../../../src/infraestructure/adapters/input/api/recipes';
import { getRecipesService, getAuthService } from '../../../../../src/infraestructure/dependencies/services';
...
describe('Recipes Controllers', () => {
    ...
    describe('getRecipeController', () => {
        it('should retrieve a recipe by id successfully', async () => {
            ...
            const mockRecipesService = {
                getRecipeById: jest.fn().mockResolvedValue({ id: 1, name: 'Pasta', ingredients: [] }),
            };

            (req.params as Record<string, string>).id = '1';

            (getAuthService as jest.Mock).mockResolvedValue(mockAuthService);
            (getRecipesService as jest.Mock).mockResolvedValue(mockRecipesService);
            ...
            expect(res.end).toHaveBeenCalledWith(JSON.stringify({ id: 1, name: 'Pasta', ingredients: [] }));
        });
        ...
        it('should return error for missing id parameter', async () => {
            ...
            expect(res.end).toHaveBeenCalledWith(JSON.stringify({ message: 'Missing id parameter' }));
        });
        ...
        it('should return error for invalid id parameter', async () => {
            ...
            expect(res.statusCode).toBe(400);
            expect(res.end).toHaveBeenCalledWith(JSON.stringify({ message: 'Invalid id parameter' }));
        });
    });
    ...
    describe('listAllRecipesController', () => {
        it('should list all recipes successfully', async () => {
            ...
            const mockRecipesService = {
                listAllRecipes: jest.fn().mockResolvedValue([{ id: 1, name: 'Pasta' }, { id: 2, name: 'Pizza' }]),
            };
            const mockFilters = { page: 1, perPage: 10 };

            (RecipeFilterValidator.validate as jest.Mock).mockReturnValue(mockFilters);
            ...
            expect(db.connect).toHaveBeenCalled();
            expect(mockAuthService.verifyHeader).toHaveBeenCalledWith(req);
            expect(RecipeFilterValidator.validate).toHaveBeenCalledWith(req.query);
            expect(getRecipesService).toHaveBeenCalledWith(mockClient);
            expect(mockRecipesService.listAllRecipes).toHaveBeenCalledWith(mockFilters);
            expect(mockClient.release).toHaveBeenCalled();
            expect(res.statusCode).toBe(200);
            expect(res.end).toHaveBeenCalledWith(JSON.stringify([{ id: 1, name: 'Pasta' }, { id: 2, name: 'Pizza' }]));
        });
    });
});