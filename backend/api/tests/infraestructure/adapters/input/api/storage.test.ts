...
import { getStorageService, getAuthService } from '../../../../../src/infraestructure/dependencies/services';
    ...
    describe('listAllIngredients', () => {
        it('should list all ingredients successfully', async () => {
            ...
            const mockStorageService = {
                getAllIngredients: jest.fn().mockResolvedValue([{ id: 1, name: 'Sugar', quantity: 100 }]),
            };

            const mockFilters = { page: 1, perPage: 10 };
            (IngredientFilterValidator.validate as jest.Mock).mockReturnValue(mockFilters);
            ...
            expect(IngredientFilterValidator.validate).toHaveBeenCalledWith(req.query);
            expect(getStorageService).toHaveBeenCalledWith(mockClient);
            expect(mockStorageService.getAllIngredients).toHaveBeenCalledWith(mockFilters);
            expect(mockClient.release).toHaveBeenCalled();
            expect(res.statusCode).toBe(200);
            expect(res.end).toHaveBeenCalledWith(JSON.stringify([{ id: 1, name: 'Sugar', quantity: 100 }]));
        });
    });
});