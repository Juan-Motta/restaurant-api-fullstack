...
describe('KitchenService', () => {
    ...
    describe('prepareOrder', () => {
        it('should log and do nothing if order is not found', async () => {
            ...
            expect(storageRepository.removeIngredientsFromStorage).not.toHaveBeenCalled();
        ...
        it('should prepare the order and update status', async () => {
            ...
            storageRepository.removeIngredientsFromStorage = jest.fn().mockResolvedValue(undefined);
            orderRepository.updateStatus = jest.fn().mockResolvedValue(undefined);

            await kitchenService.prepareOrder(mockOrder.id);

            expect(Logger.info).toHaveBeenCalledWith(`Removing ingredients from storage for order ${mockOrder.id}`);
            expect(storageRepository.removeIngredientsFromStorage).toHaveBeenCalledWith(mockOrder.recipe.id);
            expect(Logger.info).toHaveBeenCalledWith(`Order ${mockOrder.id} prepared`);
            expect(orderRepository.updateStatus).toHaveBeenCalledWith(mockOrder.id, OrderStatus.FINISHED);
        });
    });
});