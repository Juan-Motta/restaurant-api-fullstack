...
describe('RabbitMQListener', () => {
    ...
    describe('handler', () => {
        it('should prepare order and log it', async () => {
            ...
            (getKitchenService as jest.Mock).mockResolvedValue(mockStorageService);

            await rabbitMQListener.handler(mockMessage);

            expect(Logger.info).toHaveBeenCalledWith(`Preparing order ${mockMessage.data.orderId}`);
            ...
            expect(mockStorageService.prepareOrder).toHaveBeenCalledWith(mockMessage.data.orderId);
        ...
        it('should handle errors when preparing an order', async () => {
            ...
            (getKitchenService as jest.Mock).mockResolvedValue({
                prepareOrder: jest.fn().mockRejectedValue(new Error('Preparation error')),
            });
    ...
    describe('listen', () => {
        it('should set up the AMQP connection and channel', async () => {
            ...
            expect(mockChannel.assertQueue).toHaveBeenCalledWith('testQueue', { durable: false });
            expect(Logger.info).toHaveBeenCalledWith(`Waiting for messages in queue testQueue`);
        });
    });
});