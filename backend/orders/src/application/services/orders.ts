...
export class OrdersService {
    ...
    constructor(orderRepository: IOrderRepository, recipeRepository: IRecipeRepository, rabbitmqProducer: IRabbitMQProducer) {
    ...
    public async createOrder() {
        ...
        const createdOrder = await this.orderRepository.create(randomRecipe.id as number, OrderStatus.PREPARING)
        await this.sendCreateOrderEvent(createdOrder.id as number, OrderStatus.PREPARING)
        return createdOrder
    }
    ...
    public async sendCreateOrderEvent(orderId: number, orderStatus: OrderStatus) {
        try {
            await this.rabbitmqProducer.publish(Queues.STORE_QUEUE, {event: Events.CREATE_ORDER, data:{orderId, orderStatus}})
        } catch (error) {
