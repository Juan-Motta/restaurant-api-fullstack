...
const fetchOrders = async (page: number) => {
  ...
  try {
    const data = await getAllRecipes(page, itemsPerPage.value, idRecipeFilter.value, idRecipeNameFilter.value);
...
<template>
  <div class="bg-white/50 py-5 px-5 border-shadow rounded-xl h-full flex flex-col">
    ...
    <FilterBarItem @clear="clearFilters" @reload="reload">
      <input type="text" v-model="idRecipeFilter" :placeholder="$t('filter-by-recipe-id')"
      ...
      <input type="text" v-model="idRecipeNameFilter" :placeholder="$t('filter-by-recipe-name')"
        class="border w-full rounded-lg py-1 px-2 border-gray-400 placeholder-gray-400 focus:border-[#00d6bcca] focus:outline-none focus:ring-0" />
    </FilterBarItem>
    ...
    <div v-else class="flex-1 mt-4 overflow-auto">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <RecipeCardItem v-for="recipe in recipes" :key="recipe.id" :name="recipe.name" :imageUrl="recipe.imageUrl"
